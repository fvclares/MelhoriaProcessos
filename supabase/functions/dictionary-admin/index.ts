...
Deno.serve(async (request) => {
  ...
  if (companyError || !companyId) return response(403, { error: { message: "Selecione uma empresa ativa. Use o seletor de empresa." } }, requestOrigin);
  ...
  if (!adminCheck || adminCheck.role !== "admin") return response(403, { error: { message: "Este usuário não possui acesso administrativo nesta empresa." } }, requestOrigin);
